...
def add_custom_split_legend(ax, color_manager, lrs, rhos, show_styles=False):
    ...
    if show_styles:
        legend_elements.append(
            Line2D(
                [0], [0], color="black", linestyle="-", linewidth=2, label="SAM (Solid)"
            )
        )
        legend_elements.append(
            Line2D(
    ...
    if lrs:
        ...
        for lr in lrs:
            ...
            legend_elements.append(
                Line2D([0], [0], color=c, linewidth=3, label=f"  lr={lr}")
            )
    ...
    if len(valid_rhos) > 1 or (len(valid_rhos) == 1 and valid_rhos[0] > 0):
        ...
        for rho in valid_rhos:
            ...
            legend_elements.append(
                Line2D([0], [0], color=c, linewidth=3, alpha=0.8, label=f"  rho={rho}")
            )
...
selected_file = st.sidebar.selectbox(
    "Select Results File", available_files, format_func=lambda x: str(x)
)
...
selected_opts = st.sidebar.multiselect(
    "Select Optimizers", all_optimizers, default=all_optimizers
)
...
if "GD" in global_experiment_choice and "SGD" not in global_experiment_choice:
    global_data_path = Path(
        "experiments/prayers/soudry_gd/2025-12-15_12-41-06/results.npz"
    )
    is_sgd_global = False
else:
    global_data_path = Path(
        "experiments/prayers/soudry_sgd/2025-12-15_13-04-25/results.npz"
    )
    is_sgd_global = True
...
def compute_ratio_metric(reader, opt1, opt2, metric, params, seed):
    ...
    try:
        ...
        ratio = np.divide(
            data1, data2, where=data2 != 0, out=np.full_like(data1, np.nan)
        )
        return ratio
    except KeyError:
        return None
...
if (
    "LossNGD" in all_optimizers_global
    and "VecNGD" in all_optimizers_global
    and "angle" in all_metrics_global
):
    for params in all_params_global.get("LossNGD", []):
        ...
        if params in all_params_global.get("VecNGD", []):
            for seed in reader_global.seeds:
                ratio = compute_ratio_metric(
                    reader_global, "LossNGD", "VecNGD", "angle", params, seed
                )
...
global_title = st.text_input(
    "Plot Title (applies to all findings)", global_default_title, key="global_title"
)
...
tabs = st.tabs(
    [
        "Trajectory Analysis (Finding 1)",
        "SAM vs Base (Finding 2)",
        "Hyperparam Grid (Finding 3)",
        "Stability Analysis",
    ]
)
...
with tabs[0]:
    ...
    with col1:
        plot_metric = st.selectbox(
            "Metric (Y-Axis)",
            all_metrics_global,
            index=all_metrics_global.index("angle")
            if "angle" in all_metrics_global
            else 0,
            key="f1_metric",
        )
    with col2:
        x_axis_type = st.radio(
            "X-Axis Scale", ["Log", "Linear"], horizontal=True, key="f1_x_scale"
        )

    strategy = PlotStrategy(
        x_scale=AxisScale.Log if x_axis_type == "Log" else AxisScale.Linear,
        y_scale=AxisScale.Log
        if "loss" in plot_metric or "distance" in plot_metric or "angle" in plot_metric
        else AxisScale.Linear,
    )
    ...
    display_base_name = (
        global_base_opt_name if global_base_opt == "GD" else global_base_opt
    )
    display_sam_name = (
        global_sam_opt_name if global_sam_opt == "SAM" else global_sam_opt
    )
    ax_gd.set_title(f"{display_base_name} (Base)", fontsize=12, pad=10)
    ax_sam.set_title(display_sam_name, fontsize=12, pad=10)

    # Set x-axis labels
    ax_gd.set_xlabel("Angle Difference (radians)", fontsize=14)
    ax_sam.set_xlabel("Angle Difference (radians)", fontsize=14)
    ...
    for params in opt_params:
        ...
        for seed in reader_global.seeds:
            try:
                data = get_metric_data_global(
                    global_base_opt, params, seed, plot_metric
                )
                steps_data = get_metric_data_global(
                    global_base_opt,
                    params,
                    seed,
                    "steps",
                    current_metric_name=plot_metric,
                )
    ...
    if count_gd > 0 or count_sam > 0:
        ...
        if global_show_legend:
            ...
            if sorted_lrs_f1:
                legend_elements.append(
                    Patch(
                        facecolor="none", edgecolor="none", label="Learning Rate (Hue):"
                    )
                )
                ...
                for lr in sorted_lrs_f1:
                    c_rgb = compute_rho_vibrancy_color(
                        lr, sample_rho, sorted_lrs_f1, sorted_rhos_f1
                    )
                    legend_elements.append(
        ...
        if global_title:
            fig.suptitle(
                global_title, fontsize=14, y=1.08 if global_show_legend else 0.98
            )
        ...
        st.download_button(
            label="Download PDF", data=img, file_name=fn, mime="application/pdf"
        )
...
with tabs[1]:
    ...
    metric_select = st.selectbox(
        "Metric",
        [m for m in all_metrics_global],
        index=all_metrics_global.index("angle") if "angle" in all_metrics_global else 0,
        key="f2_metric",
    )
    ...
    with col_f2_1:
        selected_lrs_f2 = st.multiselect(
            "Learning Rates", sorted_lrs_f2_all, default=sorted_lrs_f2_all, key="f2_lrs"
        )
    with col_f2_2:
        selected_rhos_f2 = st.multiselect(
            "Rho Values", sorted_rhos_f2_all, default=sorted_rhos_f2_all, key="f2_rhos"
        )

    # Use selected values for plotting
    sorted_lrs_f2 = sorted(selected_lrs_f2) if selected_lrs_f2 else sorted_lrs_f2_all
    sorted_rhos_f2 = (
        sorted(selected_rhos_f2) if selected_rhos_f2 else sorted_rhos_f2_all
    )
    ...
    strategy2 = PlotStrategy(
        ...
        y_scale=AxisScale.Log
        if "loss" in metric_select or "angle" in metric_select
        else AxisScale.Linear,
    )
    ...
    for params in all_params_global.get(global_base_opt, []):
        ...
        for seed in reader_global.seeds:
            try:
                ...
                x = get_metric_data_global(
                    global_base_opt,
                    params,
                    seed,
                    "steps",
                    current_metric_name=metric_select,
                )
                c = compute_rho_vibrancy_color_f2(
                    lr, 0.0, sorted_lrs_f2_all, sorted_rhos_f2_all
                )
                ax2.plot(
                    x,
                    y,
    ...
    for params in all_params_global.get(global_sam_opt, []):
        ...
        for seed in reader_global.seeds:
            try:
                ...
                c = compute_rho_vibrancy_color_f2(
                    lr, rho, sorted_lrs_f2_all, sorted_rhos_f2_all
                )
                ax2.plot(
                    x,
                    y,
    ...
    if global_show_legend:
        ...
        if sorted_lrs_f2:
            ...
            for lr in sorted_lrs_f2:
                c_rgb = compute_rho_vibrancy_color_f2(
                    lr, sample_rho, sorted_lrs_f2_all, sorted_rhos_f2_all
                )
                legend_elements.append(
                    Line2D([0], [0], color=c_rgb, linewidth=3, label=f"  lr={lr}")
                )
        ...
        if sorted_rhos_f2 and len(sorted_rhos_f2) > 1:
            ...
            for rho in sorted_rhos_f2:
                c_rgb = compute_rho_vibrancy_color_f2(
                    sample_lr, rho, sorted_lrs_f2_all, sorted_rhos_f2_all
                )
                legend_elements.append(
                    Line2D(
                        [0],
    ...
    st.download_button(
        "Download Plot PDF", data=img2, file_name=fn2, mime="application/pdf"
    )
...
with tabs[2]:
    ...
    f3_data_path = Path(
        "experiments/prayers/twolayer_gd/2025-12-15_13-55-47/results.npz"
    )
    ...
    if not f3_data_path.exists():
        ...
        if reader_f3:
            ...
            selected_optimizers_f3 = [
                opt for opt in selected_optimizers_f3 if opt in all_optimizers_f3
            ]
            ...
            grid_metric = st.selectbox(
                "Grid Metric",
                all_metrics_f3,
                index=all_metrics_f3.index("angle") if "angle" in all_metrics_f3 else 0,
                key="t3_metric",
            )
            ...
            with col_f3_1:
                grid_lrs = st.multiselect(
                    "Grid Learning Rates",
                    sorted_lrs_f3_all,
                    default=sorted_lrs_f3_all[:4]
                    if len(sorted_lrs_f3_all) > 4
                    else sorted_lrs_f3_all,
                    key="f3_lrs",
                )
            with col_f3_2:
            ...
            if grid_lrs and grid_rhos:
                    ...
                    for j, lr in enumerate(grid_lrs):
                        ...
                        for opt, params in matching_configs:
                            ...
                            for seed in reader_f3.seeds:
                                try:
                                    y = reader_f3.get_data(
                                        opt, params, seed, grid_metric
                                    )
                ...
                if global_title:
                    fig3.suptitle(
                        global_title,
                        fontsize=14,
                        y=1.13 if global_show_legend else 0.98,
                    )
...
with tabs[3]:
    ...
    with col2_f4:
        x_axis_type_f4 = st.radio(
            "X-Axis Scale", ["Log", "Linear"], horizontal=True, key="f4_x_scale"
        )

    # Title controls for each plot
    col1_title_f4, col2_title_f4 = st.columns(2)
    with col1_title_f4:
        title_base_f4 = st.text_input(
            "LossNGD/VecNGD Plot Title", global_title or "", key="f4_title_base"
        )
    with col2_title_f4:
        title_sam_f4 = st.text_input(
            "SAM_LossNGD/SAM_VecNGD Plot Title", global_title or "", key="f4_title_sam"
        )

    strategy_f4 = PlotStrategy(
        x_scale=AxisScale.Log if x_axis_type_f4 == "Log" else AxisScale.Linear,
        y_scale=AxisScale.Log
        if "loss" in ratio_metric
        or "distance" in ratio_metric
        or "angle" in ratio_metric
        else AxisScale.Linear,
    )
    ...
    has_base_ratio = (
        "LossNGD" in all_optimizers_global and "VecNGD" in all_optimizers_global
    )
    has_sam_ratio = (
        "SAM_LossNGD" in all_optimizers_global and "SAM_VecNGD" in all_optimizers_global
    )

    if not has_base_ratio and not has_sam_ratio:
        st.warning(
            "Required optimizers (LossNGD/VecNGD or SAM_LossNGD/SAM_VecNGD) not found in data."
        )
    else:
        ...
        if has_base_ratio:
            ...
            for params in common_params:
                ...
                steps = None
                for seed in reader_global.seeds:
                    ratio = compute_ratio_f4(
                        reader_global, "LossNGD", "VecNGD", ratio_metric, params, seed
                    )
                    if ratio is not None:
                        try:
                            steps_data = reader_global.get_data(
                                "LossNGD", params, seed, "steps"
                            )
                ...
                if run_data and steps is not None:
                    mean_data = np.mean(np.stack(run_data), axis=0)
                    c = compute_rho_vibrancy_color_f4(
                        lr, rho, sorted_lrs_f4, sorted_rhos_f4
                    )
            ...
            if global_show_legend and count_base > 0:
                ...
                if sorted_lrs_f4:
                    ...
                    for lr in sorted_lrs_f4:
                        c_rgb = compute_rho_vibrancy_color_f4(
                            lr, sample_rho, sorted_lrs_f4, sorted_rhos_f4
                        )
                        legend_elements.append(
                            Line2D(
                                [0], [0], color=c_rgb, linewidth=3, label=f"  lr={lr}"
                            )
                        )
            ...
            if title_base_f4:
                fig_base.suptitle(
                    title_base_f4, fontsize=14, y=1.08 if global_show_legend else 0.98
                )
            ...
            st.download_button(
                label="Download LossNGD/VecNGD Ratio PDF",
                data=img_base,
                file_name=fn_base,
                mime="application/pdf",
                key="download_base_ratio",
            )
        ...
        if has_sam_ratio:
            ...
            common_sam_params = [
                p for p in sam_lossngd_params if p in sam_vecngd_params
            ]
            ...
            if global_show_legend and count_sam > 0:
                ...
                if sorted_rhos_f4 and len(sorted_rhos_f4) > 1:
                    ...
                    for rho in sorted_rhos_f4:
                        c_rgb = compute_rho_vibrancy_color_f4(
                            sample_lr, rho, sorted_lrs_f4, sorted_rhos_f4
                        )
                        legend_elements.append(
                            Line2D(
                                [0],
            ...
            if title_sam_f4:
                fig_sam.suptitle(
                    title_sam_f4, fontsize=14, y=1.08 if global_show_legend else 0.98
                )
            ...
            st.download_button(
                label="Download SAM_LossNGD/SAM_VecNGD Ratio PDF",
                data=img_sam,
                file_name=fn_sam,
                mime="application/pdf",
                key="download_sam_ratio",
            )
